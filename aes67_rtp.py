...
class RTPMonitorSender:
    """RTP sender for monitor stream: L16/48k/2ch (more VLC-friendly)."""

    def __init__(self, mcast_ip: str, mcast_port: int, ssrc: int, payload_type: int = 97, ttl: int = 16):
        self.mcast_ip = mcast_ip
        self.mcast_port = int(mcast_port)
        self.payload_type = int(payload_type) & 0x7F
        self.ssrc = ssrc & 0xFFFFFFFF
        self.seq = 0
        self.timestamp = 0
    ...
    @staticmethod
    def _pack_l16_from_i32le(pcm_i32_le: bytes) -> bytes:
        """Convert interleaved int32 LE to 16-bit signed big-endian."""
        out = bytearray()
        for i in range(0, len(pcm_i32_le), 4):
            sample = struct.unpack_from("<i", pcm_i32_le, i)[0]
            s16 = sample >> 16
            if s16 < -32768:
                ...
                s16 = 32767
            out.extend(struct.pack("!h", int(s16)))
        return bytes(out)

    def send_int32le_frame(self, pcm_i32_le: bytes, samples_per_channel: int):
        v_p_x_cc = 0x80
        m_pt = self.payload_type
        header = struct.pack(
            "!BBHII",
            v_p_x_cc,
            m_pt,
        ...
        payload = self._pack_l16_from_i32le(pcm_i32_le)
